...
```
key = HKDF(JWT_SECRET, salt="ciso-suite/jwt-key/v1", info="ciso-module:compliance")
```
...
`MODULE_NAME=compliance` in `docker-compose.yml` is what selects that key and that
...
derivation exists to prevent. `MODULE_COOKIE=compliance_token` likewise keeps the
...
./backup.sh                      # → backups/compliance_<date>.sql.gz
./backup.sh --dir /srv/backups --keep 30

# Scheduled (cron) — daily at 02:00
0 2 * * *  cd /path/to/compliance && ./backup.sh >> backups/backup.log 2>&1
...
./restore.sh backups/compliance_2026-08-13_0200.sql.gz
```
...
- A dump taken on an **older** application version restores fine (Alembic
  ...
  `/api/version`).
- Volume snapshots of `compliance-db-data` (VM/SAN level) also work as a
...
```ini
# /etc/systemd/system/compliance-backup.service
[Service]
Type=oneshot
WorkingDirectory=/path/to/compliance
ExecStart=/path/to/compliance/backup.sh

# /etc/systemd/system/compliance-backup.timer
[Timer]
OnCalendar=*-*-* 02:00:00
Persistent=true
[Install]
