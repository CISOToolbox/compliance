...
(function () {
    ...
    async function _runComplianceSuggest(fwId, idx, customInstruction) {
        ...
        try {
            ...
            var raw = await window._aiCallAPI(sysPrompt, userPrompt);
            if (!raw) {
                window._aiClosePanel();
                return;
            }
            var parsed = window._aiParseJSON(raw);
            var suggestions = Array.isArray(parsed) ? parsed : [parsed];
            window._aiRenderCards({
                suggestions: suggestions,
                title: "✨ AI — " + ref,
                extraHTML: '<div style="margin-top:8px"><label class="settings-label fs-xs">' + t("ai.refine_label") + '</label>' +
                    '<div style="display:flex;gap:8px"><input type="text" id="ai-suggest-refine" class="settings-input" style="flex:1" placeholder="' + esc(t("ai.refine_placeholder")) + '" />' +
                    '<button class="ai-btn-accept" id="ai-suggest-refine-run">' + t("ai.refine_run") + '</button></div></div>',
                onRendered: function () {
    ...
    async function _runGlobalAnalysisBatched() {
        ...
        if (resultEl)
            resultEl.innerHTML = '';
        var lang = _locale === "en" ? "English" : "French";
        var systemPrompt = "You are a compliance auditor. You analyze a document to identify security measures already in place and gaps. " +
        ...
        for (var b = 0; b < exigSummary.length; b += _BATCH_SIZE) {
            ...
            if (resultEl) {
                resultEl.insertAdjacentHTML("beforeend", '<div class="ai-card" style="background:#f0f4ff;padding:10px;margin-bottom:8px"><span class="fs-sm">' +
            ...
            try {
                ...
                if (!raw)
                    continue;
                var updates = window._aiParseJSON(raw);
                ...
                updates.forEach(function (u) {
                    ...
                    cardH += '</div>';
                    if (u.ecart)
                        cardH += '<div class="fs-xs" style="color:var(--text-muted);margin-bottom:4px">' + esc(u.ecart) + '</div>';
                ...
                if (resultEl)
                    resultEl.insertAdjacentHTML("beforeend", '<p class="ai-error" style="font-size:0.85em;margin:8px 0">' + esc(t("ai.error", { msg: "Batch " + batchNum + " — " + e.message })) + '</p>');
            }
        }
        if (stopBtn)
    ...
    async function _runGlobalCustom() {
        ...
        var scopePrompt = "You are a compliance assistant. The user gave an instruction that may affect some or all requirements. " +
        ...
        var targetRefs = null;
        try {
            var scopeRaw = await window._aiCallAPI(scopePrompt, scopeUser);
            if (scopeRaw) {
                var scopeParsed = window._aiParseJSON(scopeRaw);
        ...
        _globalUpdates = [];
        var systemPrompt = "You are a cybersecurity compliance expert. " +
        ...
        for (var b = 0; b < filteredExigs.length; b += batchSize) {
            ...
            try {
                ...
                if (resultEl)
                    resultEl.insertAdjacentHTML("beforeend", '<p class="ai-error" style="font-size:0.85em;margin:8px 0">' + esc(t("ai.error", { msg: e.message })) + '</p>');
            }
        }
        if (stopBtn)
            stopBtn.style.display = "none";
