# End-to-end tests - Compliance (standalone)

These tests drive a **real running Compliance stack** over HTTP. Nothing is mocked.
...
```bash
docker compose up -d
E2E_BASE_URL=http://localhost:8082 python3 -m pytest -v tests/e2e
```
...
|----------|---------|---------|
| `E2E_BASE_URL` | `http://localhost:8082` | where the module answers |
...
(`/compliance/`). A standalone deployment has none of that - it serves the module at
`/`, mints a `compliance_token` cookie signed with a key derived per module, and logs
