...
function _showPropositionsModal(fwId, idx, exigRef, entry, available) {
    ...
    available.forEach(function (mt, i) {
        ...
        if (mt.details)
            h += '<div class="text-muted fs-xs" style="margin-top:4px">' + esc(_rt(mt, "details").substring(0, 200)) + (mt.details.length > 200 ? "…" : "") + '</div>';
        h += '</div>';
...
function _showMesureModal() {
    ...
    if (!isDraft) {
        ...
        (m.preuves_ids || []).forEach(function (pid) {
            var p = _getPreuve(pid);
            if (p)
                h += '<div class="linked-tag"><span style="cursor:pointer" data-click="_goEditPreuveFromMesure" data-args=\'' + _da(_draftMesureFwId || "", mid, pid) + '\'>' + esc(p.id) + ' ' + esc(p.label) + '</span><span class="tag-x" data-click="_unlinkPreuve" data-args=\'' + _da(mid, pid, _draftMesureFwId || "") + '\' data-stop>&times;</span></div>';
        });
...
function _renderFwPreuves(fwId, label) {
    ...
    if (preuves.length > 0) {
        ...
        preuves.forEach(p => {
            ...
            let statut = "—";
            if (dst === "expired")
                statut = badgeTone(t("comp.prv.expiree"), "critical");
            else if (dst === "soon")
                statut = badgeTone(t("comp.prv.bientot"), "high");
            else if (dst === "valid")
                statut = badgeTone(t("comp.prv.ok"), "low");
...
function renderControles() {
    ...
    if (rows.length === 0) {
        ...
        if (retards > 0)
            h += `<div class="synth-card mb-16" style="border-color:var(--red);background:var(--ct-critical-tint)"><p style="color:var(--red);font-weight:600">${t("comp.ctrl.alertes", { count: retards })}</p></div>`;
