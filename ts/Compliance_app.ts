...
function selectPanel(panelId: string) {
    ...
    if (panelId.startsWith("fw:")) {
        ...
        const afterLoad = () => {
            // BUG-16: hydrate an active framework's working-set now that
            // _ensureFramework has loaded its measures into REFERENTIELS_META,
...
// haven't been hydrated yet. No-op for anssi/iso and when already populated.
...
function _showMesureModal() {
    ...
    if (isDraft) {
        ...
        h += '<button class="btn-add fs-xs" style="background:var(--red)" data-click="_deleteMesureModal" data-args=\'' + _da(mid) + '\'>' + esc(t("comp.mes.btn_supprimer")) + '</button>';
    ...
    if (!isDraft) {
        ...
        (m.preuves_ids||[]).forEach(function(pid) {
            var p = _getPreuve(pid);
            if (p) h += '<div class="linked-tag"><span style="cursor:pointer" data-click="_goEditPreuveFromMesure" data-args=\'' + _da(_draftMesureFwId||"",mid,pid) + '\'>' + esc(p.id) + ' ' + esc(p.label) + '</span><span class="tag-x" data-click="_unlinkPreuve" data-args=\'' + _da(mid,pid,_draftMesureFwId||"") + '\' data-stop>&times;</span></div>';
        });
        ...
        h += '<button class="btn-add fs-xs" style="margin-left:4px" data-click="_createAndLinkPreuve" data-args=\'' + _da(mid,_draftMesureFwId||"") + '\'>' + esc(t("comp.mes.btn_nouvelle_preuve")) + '</button>';
    } else {
