...
function _renderFwPreuves(fwId, label) {
    ...
    if (preuves.length > 0) {
        ...
        preuves.forEach(p => {
            ...
            let statut = "—";
            if (dst === "expired")
                statut = badge(t("comp.prv.expiree"), "var(--red)");
            else if (dst === "soon")
                statut = badge(t("comp.prv.bientot"), "var(--orange)");
            else if (dst === "valid")
                statut = badge(t("comp.prv.ok"), "var(--green)");
...
function renderControles() {
    ...
    if (rows.length === 0) {
        ...
        if (retards > 0)
            h += `<div class="synth-card mb-16" style="border-color:var(--red);background:#fef2f2"><p style="color:var(--red);font-weight:600">${t("comp.ctrl.alertes", { count: retards })}</p></div>`;
