...
let D: ComplianceData = JSON.parse(JSON.stringify(window.COMPLIANCE_INIT_DATA || {}));
...
function _getMesuresTypesFor(fwId: string, exigRef: string) {
...
// ── Reference measure catalog (in-house) ────────────────────────────────
let _referenceControlsLoaded = false;
function _ensureReferenceControls(cb: () => void) {
    if (_referenceControlsLoaded) { cb(); return; }
...
function _getReferenceControlsFor(fwId: string, exigRef: string): ComplianceReferenceControl[] {
...
// Converts a reference measure into a "template measure" consumable by the
// existing proposal modal (shared through exigences = framework_refs).
function _refControlToMesureType(rc: ComplianceReferenceControl): ComplianceMesureType {
...
function _openPropositions(fwId: string, idx: number, exigRef: string, entry: ComplianceExigence, types: ComplianceMesureType[]) {
    ...
    if (available.length === 0) {
        showStatus(t("comp.alert.all_linked", {count: types.length, ref: exigRef}), "info" as unknown as boolean);
        return;
    }
...
// Reference measure catalog (in-house) first; fall back to the older
// COMPLIANCE_MESURES_TYPES catalog for what it does not cover yet.
function _proposerMesures(fwId: string, idx: number) {
    _ensureReferenceControls(() => {
        ...
        _ensureMesuresTypes(() => {
            ...
            if (types.length === 0) {
                showStatus(t("comp.alert.no_mesure_type", {ref: exigRef}), "warn" as unknown as boolean);
                return;
            }
...
function _showPropositionsModal(fwId: string, idx: number, exigRef: string, entry: ComplianceExigence, available: ComplianceMesureType[]) {
...
window._acceptProposition = function(i: number) {
    var ctx = window._propositionsCtx;
    if (!ctx) return;
    var mt = ctx.available[i];
    if (!mt) return;
    _applyProposition(ctx, mt);
    var card = document.querySelectorAll<HTMLElement>("#propositions-modal .proposition-card")[i];
    if (card) {
...
window._rejectProposition = function(i: number) {
    var card = document.querySelectorAll<HTMLElement>("#propositions-modal .proposition-card")[i];
    if (card) {
...
window._acceptAllPropositions = function() {
    ...
    if (!ctx) return;
    _saveState();
    var cards = document.querySelectorAll<HTMLElement>("#propositions-modal .proposition-card");
...
function _applyProposition(ctx: CompliancePropositionsCtx, mt: ComplianceMesureType) {
    var entry = ctx.entry;
...
function _getMesure(id: string) { return D.mesures.find(m => m.id === id); }
function _getPreuve(id: string) { return D.preuves.find(p => p.id === id); }
function _findMesuresForPreuve(preuveId: string) { return D.mesures.filter(m => (m.preuves_ids||[]).includes(preuveId)).map(m => m.id); }
...
function _getExigRef(fwId: string, entry: ComplianceExigence) {
    return entry.ref || "";
}

// Measures linked to a framework (at least one requirement of that fw)
function _getMesuresForFw(fwId: string) {
...
function _getPreuvesForFw(fwId: string) {
...
// Status labels
function _normStatut(key: string) {
    var k = String(key || "").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim();
    var map: Record<string, string> = { "planifie": "planifie", "en cours": "en_cours", "en_cours": "en_cours",
                "termine": "termine", "terminee": "termine", "terminees": "termine",
                "preuve manquante": "preuve_manquante", "preuve_manquante": "preuve_manquante" };
    return map[k] || key;
}
function _statutLabel(key: string) { return t("comp.statut." + _normStatut(key)) || key; }
const _statutColors: Record<string, string> = {planifie:"orange",en_cours:"blue",termine:"green",preuve_manquante:"red"};

// ── Automatic status computation ─────────────────────────────────────

// Effective status of a measure (accounts for proof expiry)
function _mesureEffectiveStatut(m: ComplianceMesure) {
    if (m.statut !== "termine") return m.statut || "planifie";
    // Done: check there is at least one valid (non-expired) proof
    const preuves = (m.preuves_ids || []).map(id => _getPreuve(id)).filter(Boolean) as CompliancePreuve[];
...
// Derived from the register at render time; the exported data never carries
// the status itself.
function _exigDerogation(fwId: string, entry: ComplianceExigence): CtDerRecord | null {
    const key = fwId + ":" + (entry.ref || "");
    const live = (D.derogations || []).filter(d => d.status === "approved" && d.subject_type === "control" && d.subject_id === key);
    return live.length ? live[0] : null;
}

// Requirement status: OK if ≥1 measure AND all done (with valid proofs)
function _exigenceStatut(entry: ComplianceExigence, fwId?: string) {
    if (entry.applicable === false || entry.applicable === "non") return "na";
    if (fwId && _exigDerogation(fwId, entry)) return "derogated";
    const ids = entry.mesures_ids || [];
    if (ids.length === 0) return "ko";
    const mesures = ids.map(id => _getMesure(id)).filter(Boolean) as ComplianceMesure[];
    if (mesures.length === 0) return "ko";
    const allOk = mesures.every(m => _mesureEffectiveStatut(m) === "termine");
    return allOk ? "ok" : "ko";
}

function _exigStatutLabel(key: string) { return t("comp.exig_statut." + key) || key; }
const _exigStatutColors: Record<string, string> = {ok: "green", ko: "red", na: "gray", derogated: "gray"};
...
// family.
const _CT_TONES: Record<string, string> = {
...
const _recJours: Record<string, number> = {ponctuel:0,mensuelle:30,trimestrielle:90,semestrielle:180,annuelle:365};

// Base frameworks (ANSSI, ISO) with the same structure as the additional ones for the UI
const _BASE_FRAMEWORKS: Record<string, ComplianceFwMeta> = {
...
function _getAllFrameworks(): Record<string, ComplianceFwMeta> {
    var all: Record<string, ComplianceFwMeta> = Object.assign({}, _BASE_FRAMEWORKS, REFERENTIELS_META);
...
function selectPanel(panelId: string) {
    ...
    if (_exigMarked && !_exigTarget && panelId !== "fw:" + _exigMarked.split(":")[0] + ":exigences") _exigMarked = "";
    if (panelId.startsWith("fw:")) {
        ...
        else if (panelId === "nonconformities") renderNonconformities();
    }
...
function ensureKeys() {
    ...
    // FEAT-45 — the register. Past its end of validity a derogation expires
    // on load: no scheduler here, the file is only alive when it is open.
    if (!Array.isArray(D.nonconformities)) D.nonconformities = [];
    if (!Array.isArray(D.derogations)) D.derogations = [];
    if (window.ct_nonconformity_local) window.ct_nonconformity_local.expire(_ncSpec());
    ...
    const initData: Partial<ComplianceInitData> = window.COMPLIANCE_INIT_DATA || {};
...
function renderContext() {
    ...
    for (const [key, tKey] of [["societe","comp.context.organisation"],["date_evaluation","comp.context.date"],["evaluateur","comp.context.evaluateur"],["perimetre","comp.context.perimetre"]]) {
        ...
        h += `<div class="ct-meta-item mb-12"><div class="label">${label}</div><div class="value">
            <input type="text" value="${esc((m as unknown as Record<string, string>)[key])}" class="w-full" data-change="_setMeta" data-args='${_da(key)}' data-pass-value />
        </div></div>`;
    }
...
function _setMeta(key: string, val: string) {
    _saveState(); (D.meta as unknown as Record<string, string>)[key] = val;
...
// populated.
function _hydrateFwFromMeta(fwId: string): void {
    if (!fwId || fwId === "anssi" || fwId === "iso") return;
    ...
    if (Array.isArray(existing) && existing.length) {
        ...
        const byRef: Record<string, typeof measures[number]> = {};
        measures.forEach(m => { byRef[m.ref] = m; });
        existing.forEach(e => {
            const m = byRef[e.ref];
            if (!m) return;
            if (!e.theme && m.theme) e.theme = m.theme;
            if (!e.theme_en && m.theme_en) e.theme_en = m.theme_en;
            if (!e.mesure && m.mesure) e.mesure = m.mesure;
            if (!e.mesure_en && m.mesure_en) e.mesure_en = m.mesure_en;
            if (!e.description && m.description) e.description = m.description;
            if (!e.description_en && m.description_en) e.description_en = m.description_en;
        });
    ...
    if (initRefs[fwId]) { D.referentiels[fwId] = JSON.parse(JSON.stringify(initRefs[fwId])); return; }
...
function toggleReferentiel(fwId: string) {
    const doToggle = () => {
        _saveState();
...
function importCustomCSV() {
    var fi = document.createElement("input");
        ...
        reader.onload = function() {
            _parseAndImportCSV(reader.result as string, file.name);
        };
...
function _parseAndImportCSV(csvText: string, filename: string) {
    var parsed = _parseCSV(csvText);
    var headers = parsed.headers;
    var rows = parsed.rows;
    if (headers.length === 0 || rows.length === 0) { showStatus(t("comp.csv.error_empty")); return; }
    ...
    // Parse rows
    var measures: ComplianceRefMeasure[] = [];
...
function renderDashboard() {
    let h = "";
    const frameworks: { fwId: string; label: string; total: number; ok: number; ko: number; derog: number; pct: number; excluded: number }[] = [];
    for (const fwId of D.referentiels_actifs) {
        const exigences = _getExigences(fwId);
...
// ── Per-framework view ────────────────────────────────────────────
function _renderFwView(fwId: string, subview: string | null) {
    const meta = _getAllFrameworks()[fwId];
...
function _renderFwDashboard(fwId: string, label: string) {
    const exigences = _getExigences(fwId);
    const applicable = exigences.filter(e => e.applicable !== false && e.applicable !== "non");
    const ok = applicable.filter(e => _exigenceStatut(e, fwId) === "ok").length;
    const derog = applicable.filter(e => _exigDerogation(fwId, e) !== null).length;
...
// ── Requirements ──────────────────────────────────────────────────
let _exigFilter = "";
/** Requirement to scroll to on the next render of its framework ("ref"). */
let _exigTarget = "";
/** Requirement kept highlighted while its view is on screen ("fw:ref"). */
let _exigMarked = "";

function _filterExigences(fwId: string, val: string) {
    _exigFilter = val;
    _renderFwView(fwId, "exigences");
}

function _renderFwExigences(fwId: string, label: string) {
    ...
    const exigences: { entry: ComplianceExigence; origIdx: number }[] = [];
    ...
    exigences.forEach((item) => {
        ...
        const linkedMesures = (e.mesures_ids || []).map(id => _getMesure(id)).filter(Boolean) as ComplianceMesure[];
        ...
        h += `<tr${notApplicable?' class="ct-bg-alt"':''} data-exig-ref="${esc(ref)}">`;
    ...
    document.querySelectorAll<HTMLTextAreaElement>("#fw-content textarea").forEach(function(ta) {
        if (ta.value) { ta.style.height = "auto"; ta.style.height = ta.scrollHeight + "px"; }
    });
    // A requirement opened from a record (?req=) is highlighted, and scrolled
    // to once; the marker survives the re-renders of the same view.
    var scrollTo = _exigTarget;
    if (_exigTarget) { _exigMarked = fwId + ":" + _exigTarget; _exigTarget = ""; }
    if (_exigMarked.indexOf(fwId + ":") === 0) {
        var target = _exigMarked.substring(fwId.length + 1);
        var rows = document.querySelectorAll<HTMLTableRowElement>("#exig-" + fwId + "-table tr[data-exig-ref]");
        for (var ri = 0; ri < rows.length; ri++) {
            if (rows[ri].getAttribute("data-exig-ref") !== target) continue;
            rows[ri].classList.add("ct-row-target");
            if (scrollTo) rows[ri].scrollIntoView({ block: "center" });
            break;
        }
    }
}
...
function _toggleApplicable(fwId: string, idx: number, checked: boolean) {
    _saveState();
    const entry = _getExigEntry(fwId, idx);
...
function _updateExig(fwId: string, idx: number, field: string, val: string) {
...
function _getExigEntry(fwId: string, idx: number): ComplianceExigence {
    return ((D.referentiels && D.referentiels[fwId]) || [])[idx] || ({} as ComplianceExigence);
}

// ═══════════════════════════════════════════════════════════════════════
// NON-CONFORMITIES AND DEROGATIONS (FEAT-45)
// ═══════════════════════════════════════════════════════════════════════
// The same register as the module, on the blob instead of a server:
// ct_nonconformity draws it, ct_nonconformity_local holds the rules, and the
// person holding the file declares, qualifies and approves — there is no
// second account to ask (local approval). A requirement is named
// "<framework>:<ref>" here as there, so a record keeps its meaning.
function _ncSpec(): CtNcLocalSpec {
    return {
        records: function() { if (!Array.isArray(D.nonconformities)) D.nonconformities = []; return D.nonconformities; },
        derogations: function() { if (!Array.isArray(D.derogations)) D.derogations = []; return D.derogations; },
        save: function() { _autoSave(); },
        actor: function() { return (D.meta && D.meta.evaluateur) || ""; },
        subjectType: "control",
        maxDays: function() { return (D.meta && D.meta.max_derogation_days) || 365; },
        setMaxDays: function(days: number) { _saveState(); D.meta.max_derogation_days = days; },
            ...
            options: function() {
                ...
                D.referentiels_actifs.forEach(function(fwId) {
                    ...
                    _getExigences(fwId).forEach(function(e) {
                        if (e.applicable === false || e.applicable === "non") return;
...
function _ncOptions(): CtNcOptions {
    ...
    // A decision on a derogation changes a requirement's status: the view
    // showing it is redrawn.
    opts.onChange = function() { _autoSave(); if (_currentFw && _currentSubview) _renderFwView(_currentFw, _currentSubview); };
    return opts;
}

function renderNonconformities() {
    if (!window.ct_nonconformity) return;
    window.ct_nonconformity.renderPanel(document.getElementById("nonconformities-content")!, _ncOptions());
}

// Corrective measure of a non-conformity: the same unified modal as the rest
// of the action plan, promise-returning and without a requirement link.
function _createMesureForNc(prefill: { title: string; description: string }): Promise<CtNcItemOption | null> {
    if (!window.ct_measure_modal) return Promise.resolve(null);
    var statusOpts = [
    ...
    return window.ct_measure_modal.open({ description: prefill.title, details: prefill.description } as any, {
        title: t("comp.mes.new_draft"),
        saveLabel: t("comp.mes.btn_valider"),
        ...
        if (!result || result.__deleted || result.__advanced) return null;
        _saveState();
        var created: ComplianceMesure = {
            id: _genMesureId(),
        ...
        _autoSave();
        showStatus(t("comp.status.mesure_created"));
...
function _ensureInternalFramework(): void {
    ...
    if (!D.referentiels[_INTERNAL_FW]) D.referentiels[_INTERNAL_FW] = [];
    if (D.referentiels_actifs.indexOf(_INTERNAL_FW) < 0) D.referentiels_actifs.push(_INTERNAL_FW);
}

function _createControlForNc(nc: { title: string; description: string; domain: string }): Promise<CtNcItemOption | null> {
    if (!window.ct_modal) return Promise.resolve(null);
    var existing = D.referentiels[_INTERNAL_FW] || [];
    var n = existing.length + 1;
    var ref = "INT-" + String(n).padStart(3, "0");
    while (existing.some(function(e) { return e.ref === ref; })) { n++; ref = "INT-" + String(n).padStart(3, "0"); }
    ...
    return window.ct_modal.open({ title: t("comp.nc.create_control"), body: h, size: "md", buttons: [
        ...
        { id: "ok", label: t("comp.nc.create_control"), primary: true, result: function() {
            var r = ((document.getElementById("ct-cc-ref") as HTMLInputElement).value || "").trim();
            var m = ((document.getElementById("ct-cc-mesure") as HTMLTextAreaElement).value || "").trim();
            if (!r || !m) { showStatus(t("comp.nc.create_control_missing"), true); return false; }
            if (!/^[A-Za-z0-9._-]+$/.test(r)) { showStatus(t("comp.nc.create_control_badref"), true); return false; }   // a key: "fw:ref"
            if (existing.some(function(e) { return e.ref === r; })) { showStatus(t("comp.nc.create_control_dup", { ref: r }), true); return false; }
            return { ref: r, theme: ((document.getElementById("ct-cc-theme") as HTMLInputElement).value || "").trim(), mesure: m };
        } }
    ] }).then(function(res: any) {
        if (!res) return null;
        _saveState();
        _ensureInternalFramework();
        var entry: ComplianceExigence = { ref: res.ref, thematique: res.theme, theme: res.theme, mesure: res.mesure,
...
function _openRequirement(key: string): boolean {
    var fwId = key.split(":")[0];
    if (!fwId || D.referentiels_actifs.indexOf(fwId) < 0) { showStatus(t("comp.nc.fw_inactive"), true); return false; }
    _exigFilter = "";
...
function _exigSubject(fwId: string, idx: number) {
...
function _declareNcExig(fwId: string, idx: number) {
...
function _requestDerogExig(fwId: string, idx: number) {
    const s = _exigSubject(fwId, idx);
...
function _linkExistingMesure(fwId: string, idx: number, mesureId: string) {
    if (!mesureId) return;
    _saveState();
    const entry = _getExigEntry(fwId, idx);
    if (!entry.mesures_ids) entry.mesures_ids = [];
...
function _createAndLinkMesure(fwId: string, idx: number) {
...
function _unlinkMesure(fwId: string, idx: number, mesureId: string) {
    _saveState();
    const entry = _getExigEntry(fwId, idx);
...
let _editingMesure: string | null = null;
let _mesureEditReturnTo: string | null = null; // "fw:anssi:exigences" when coming from the requirements

let _mesureFilter = "";

function _renderFwMesures(fwId: string, label: string) {
    // Only show the measures linked to the current framework
    const fwMesureIds = new Set<string>();
        ...
        h += window.ct_table!.render({
            ...
            columns: [
                ...
                { key: "statut",      label: t("comp.mes.col_statut"),      width: "100px",
                  render: m => _mesureBadge(m as ComplianceMesure) },
...
window._refreshMeasures = function() {
    ...
    if (pid && window.ComplianceAPI && window.ComplianceAPI.get) {
        window.ComplianceAPI.get(pid).then(function(proj) {
            if (proj && proj.data) {
                var data = proj.data;
                Object.keys(data).forEach(function(k) { (D as unknown as Record<string, unknown>)[k] = data[k]; });
            }
...
window._editMesureRow = function(row: { id: string; __fwId?: string | null }) {
    if (!row || !row.id) return;
    if (!window.ct_measure_modal) { _editMesure(row.__fwId || null, row.id); return; }
    ...
    window.ct_measure_modal!.open(m, {
        ...
        onAddNote: function(_entry, fullLog) {
            (m as unknown as Record<string, unknown>).progress_log = fullLog;
            if (typeof _persist === "function") _persist("measure", m.id, { progress_log: fullLog });
        },
        extraFields: [
        ...
        ["description", "details", "statut", "responsable", "date_cible", "recurrence", "dernier_controle", "progress_log"].forEach(function(k) {
            if (result[k] !== undefined && result[k] !== (m as unknown as Record<string, unknown>)[k]) {
                (m as unknown as Record<string, unknown>)[k] = result[k];
                patch[k] = result[k];
            }
        });
        // Apply pending exigence links/unlinks to D
        var dirtyControls: Record<string, ComplianceExigence> = {};
...
window._createMesureUnified = function(fwId: string | null, linkIdx: number | null) {
    if (!window.ct_measure_modal) {
        ...
        _draftMesureFwId = fwId; _draftMesureLinkIdx = linkIdx; _editingMesure = "__draft__"; _showMesureModal();
        return;
    }
    var statusOpts = [
        { value: "planifie", label: _statutLabel("planifie") },
    ...
    window.ct_measure_modal!.open(null, {
        ...
        var payload = {
            ...
            dernier_controle: result.dernier_controle || "", preuves_ids: [] as string[]
        };
        var afterCreate = function(created: ComplianceMesure) {
            D.mesures.push(created);
        ...
        if (window.ComplianceAPI && typeof window._getActiveProjectId === "function") {
            window.ComplianceAPI.createMeasure(window._getActiveProjectId(), payload).then(afterCreate)
                .catch(function(e: any) { showStatus("Erreur création : " + (e.message || e), true); });
        } else { afterCreate(payload as ComplianceMesure); _autoSave(); }
    });
};
...
function _isExigLinkedInModal(e: ComplianceExigence, fwId: string, idx: number, mesureId: string) {
    // Real state from D
...
function _renderExigsForModal(mesureId: string) {
    var h = "";
    var linked: { fwId: string; idx: number; ref: string; fwLabel: string }[] = [];
    ...
    var exigOpts: { value: string; label: string }[] = [];
...
window._linkExigInModal = function(mesureId: string, val: string) {
    if (!val) return;
    var parts = val.split(":");
    var fwId = parts[0];
...
window._unlinkExigInModal = function(mesureId: string, fwId: string, idx: number) {
    var entry = _getExigEntry(fwId, idx);
    if (!entry) return;
...
// ct_modal stays open.
function _renderPreuvesForModal(mesureId: string) {
...
window._linkPreuveInModal = function(mesureId: string, preuveId: string) {
    if (!preuveId) return;
...
window._unlinkPreuveInModal = function(mesureId: string, preuveId: string) {
...
var _ctReturnToMesureId: string | null = null;
var _ctReturnToMesureFwId: string | null = null;
...
window._editPreuveFromModal = function(mesureId: string, preuveId: string) {
...
window._createAndLinkPreuveInModal = function(mesureId: string) {
    var m = _getMesure(mesureId);
    if (!m) return;
    _saveState();
    var id = _genPreuveId();
...
window._bulkComplianceMesuresDone = function(scope: string) {
...
window._bulkComplianceMesuresDelete = function(scope: string) {
    var ids = Array.from(window.ct_bulkbar!.getSelection(scope));
    if (!ids.length) return;
    _saveState();
...
function _filterMesures(fwId: string, val: string) {
    _mesureFilter = val;
    _renderFwView(fwId, "mesures");
}

function _findExigencesForMesure(mesureId: string) {
    const result: string[] = [];
...
function _findFwsForMesure(mesureId: string) {
...
var _draftMesure: ComplianceMesureDraft | null = null;
var _draftMesureFwId: string | null = null;
var _draftMesureLinkIdx: number | null = null;
...
function _commitDraft() {
    ...
    var _afterCreate = function(created: ComplianceMesure) {
        D.mesures.push(created);
        if (linkIdx !== null && fwId) {
            var entry = _getExigEntry(fwId, linkIdx);
            if (entry) {
    ...
    if (window.ComplianceAPI && typeof window._getActiveProjectId === "function") {
        window.ComplianceAPI.createMeasure(window._getActiveProjectId(), payload).then(_afterCreate)
            .catch(function(e: any) { showStatus("Erreur creation : " + (e.message || e), true); });
    } else {
...
function _showMesureModal() {
    ...
    if (!m) return;
    var mid = isDraft ? "__draft__" : (m as ComplianceMesure).id;
    ...
    if (!isDraft) {
        ...
        (m.preuves_ids||[]).forEach(function(pid) {
            var p = _getPreuve(pid);
            if (p) h += '<div class="linked-tag"><span class="ct-clickable" data-click="_goEditPreuveFromMesure" data-args=\'' + _da(_draftMesureFwId||"",mid,pid) + '\'>' + esc(p.id) + ' ' + esc(p.label) + '</span><span class="tag-x" data-click="_unlinkPreuve" data-args=\'' + _da(mid,pid,_draftMesureFwId||"") + '\' data-stop>&times;</span></div>';
        });
...
window._deleteMesureModal = function(mesureId: string) {
...
function _addMesure(fwId: string) {
    window._createMesureUnified!(fwId, null);
}

function _editMesure(fwId: string | null, mesureId: string) {
    if (_draftMesure) _discardDraft();
...
function _renderLinkedExigences(mesureId: string, currentFwId: string | null) {
    ...
    const linked: { fwId: string; idx: number; ref: string; fwLabel: string }[] = [];
    for (const fwId of D.referentiels_actifs) {
        const exigences = _getExigences(fwId);
        const meta = _getAllFrameworks()[fwId];
    ...
    const exigOpts: { value: string; label: string }[] = [];
    for (const fwId of D.referentiels_actifs) {
        const exigences = _getExigences(fwId);
        const meta = _getAllFrameworks()[fwId];
        const fwLabel = meta ? meta.label : fwId;
...
window._linkMesureToExig = function(mesureId: string, currentFwId: string | null, val: string) {
    if (!val) return;
    _saveState();
...
window._unlinkMesureFromEdit = function(mesureId: string, fwId: string, idx: number, currentFwId: string | null) {
    _saveState();
    const entry = _getExigEntry(fwId, idx);
    entry.mesures_ids = (entry.mesures_ids || []).filter(id => id !== mesureId);
...
function _updateMesure(mesureId: string, field: string, val: string) {
    if (mesureId === "__draft__" && _draftMesure) {
        (_draftMesure as unknown as Record<string, unknown>)[field] = val;
        return;
    }
    const m = _getMesure(mesureId);
    if (m) { (m as unknown as Record<string, unknown>)[field] = val; _persist("measure", m.id, _obj(field, val)); }
}

function _deleteMesure(mesureId: string, fwId: string | null) {
    if (!confirm(t("comp.confirm.delete_mesure", {id: mesureId}))) return;
    _saveState();
...
window._linkExistingPreuve = function(mesureId: string, fwId: string, preuveId: string) {
    if (!preuveId) return;
    _saveState();
    const m = _getMesure(mesureId);
    if (m) {
...
window._unlinkPreuve = function(mesureId: string, preuveId: string, fwId: string) {
    _saveState();
    const m = _getMesure(mesureId);
    if (m) {
...
window._createAndLinkPreuve = function(mesureId: string, fwId: string) {
    _saveState();
    var id = _genPreuveId();
    var newPreuve = { id: id, label: "", url: "", date_obtention: "", date_expiration: "", commentaire: "" };
    D.preuves.push(newPreuve);
...
let _editingPreuve: string | null = null;
let _preuveEditReturnTo: string | null = null;

let _preuveFilter = "";

function _renderFwPreuves(fwId: string, label: string) {
    const fwPreuveIds = new Set<string>();
        ...
        h += window.ct_table!.render({
            ...
            columns: [
                { key: "id",  label: t("comp.prv.col_id"), width: "70px",
                  render: (p: any) => '<span class="fw-600">' + esc(p.id) + '</span>' },
                { key: "label", label: t("comp.prv.col_label"),
                  render: (p: any) => esc(p.label || "—") },
                { key: "url", label: t("comp.prv.col_url"),
                  render: (p: any) => p.url ? '<a href="' + esc(p.url) + '" target="_blank" rel="noopener noreferrer" data-stop>' + esc(String(p.url).substring(0, 40)) + '</a>' : "—" },
                { key: "date_obtention", label: t("comp.prv.col_obtention"), width: "100px",
                  render: (p: any) => esc(p.date_obtention || "—") },
                { key: "date_expiration", label: t("comp.prv.col_expiration"), width: "100px",
                  render: (p: any) => esc(p.date_expiration || "—") },
                { key: "mesures", label: t("comp.prv.col_mesures"), width: "100px",
                  render: (p: any) => '<span class="fs-xs">' + esc((p.__linked || []).join(", ") || "—") + '</span>' },
                { key: "statut", label: t("comp.prv.col_statut"), width: "80px",
                  render: (p: any) => p.__dst === "expired" ? badgeTone(t("comp.prv.expiree"), "critical")
...
window._editPreuveRow = function(row: { id: string; __fwId?: string | null }) {
    if (!row || !row.id) return;
    _editPreuve(row.__fwId || _currentFw || "", row.id);
};

window._bulkCompliancePreuvesDelete = function(scope: string) {
    var ids = Array.from(window.ct_bulkbar!.getSelection(scope));
    if (!ids.length) return;
    _saveState();
    D.mesures.forEach(function(m) {
        if (m.preuves_ids) {
            ...
            if (m.preuves_ids.length !== before) _persist("measure", m.id, { preuves_ids: m.preuves_ids });
        }
    });
    ...
    window.ct_bulkbar!.clear(scope);
    if (_currentFw) _renderFwView(_currentFw, _currentSubview);
};

function _filterPreuves(fwId: string, val: string) {
    _preuveFilter = val;
    _renderFwView(fwId, "preuves");
}

function _addPreuveGlobal(fwId: string) {
    _saveState();
...
function _editPreuve(fwId: string, preuveId: string) {
    _editingPreuve = preuveId;
    _returnToMesureId = null;
    _showPreuveModal();
}

let _returnToMesureId: string | null = null;
window._goEditPreuveFromMesure = function(fwId: string, mesureId: string, preuveId: string) {
...
function _showPreuveModal() {
    ...
    var h = '<div style="background:var(--ct-surface);border-radius:var(--ct-r-xl);padding:var(--ct-s6);max-width:520px;width:100%;max-height:90vh;overflow-y:auto;box-shadow:0 8px 32px rgba(0,0,0,0.2)">';
    h += '<div class="ct-flex ct-gap-2 ct-items-center ct-mb-4">';
    h += '<strong class="ct-flex-1 ct-text-body">' + esc(p.id) + '</strong>';
    h += '<button class="ct-btn mt-8" data-write data-variant="primary" data-size="sm" data-click="_deletePreuveModal" data-args=\'' + _da(p.id) + '\'>' + esc(t("comp.prv.btn_supprimer")) + '</button>';
    h += '<button class="ct-btn mt-8" data-write data-variant="primary" data-size="sm" data-click="_closePreuveModal">' + esc(t("comp.prv.btn_valider")) + '</button>';
    h += '</div>';
    ...
    h += '<div class="ct-flex ct-gap-2 ct-row-wrap ct-mb-2">';
    var _pkind = (p as unknown as Record<string, unknown>).kind as string || "link";
    h += '<label class="fs-xs">' + esc(t("comp.prv.label_kind")) + ' <select data-change="_updatePreuveField" data-args=\'' + _da(p.id,"kind") + '\' data-pass-value>'
        + ["link","file","observation"].map(function(k) { return '<option value="' + k + '"' + (_pkind === k ? " selected" : "") + '>' + esc(t("comp.prv.kind." + k)) + '</option>'; }).join("")
        + '</select></label>';
    h += '<label class="fs-xs">' + esc(t("comp.prv.label_owner")) + ' <input type="text" value="' + esc(String((p as unknown as Record<string, unknown>).owner || "")) + '" data-change="_updatePreuveField" data-args=\'' + _da(p.id,"owner") + '\' data-pass-value /></label>';
    h += '</div>';
    h += '<div class="ct-flex ct-gap-2 ct-row-wrap ct-mb-2">';
    ...
    ov.innerHTML = h;
    ov.addEventListener("click", function(e) { if (e.target === ov) window._closePreuveModal!(); });
    document.body.appendChild(ov);
    ov.querySelectorAll("textarea").forEach(function(ta) { _autoHeight(ta); });
}
...
window._deletePreuveModal = function(preuveId: string) {
...
window._bulkCompliancePlanDone = function(scope: string) {
    var ids = Array.from(window.ct_bulkbar!.getSelection(scope));
    if (!ids.length) return;
    _saveState();
    var count = 0;
...
window._bulkCompliancePlanDelete = function(scope: string) {
    var ids = Array.from(window.ct_bulkbar!.getSelection(scope));
    if (!ids.length) return;
    _saveState();
    for (const fwId of D.referentiels_actifs) {
...
function _filterPlan(val: string) {
    _planFilter = val;
    renderPlan();
}
...
window._linkExistingPreuvePlan = function(mesureId: string, preuveId: string) {
    if (!preuveId) return;
    _saveState();
    const m = _getMesure(mesureId);
    if (m) {
...
window._createAndLinkPreuvePlan = function(mesureId: string) {
    _saveState();
    const id = _genPreuveId();
    const newPreuve = { id, label: "", url: "", date_obtention: "", date_expiration: "", commentaire: "" };
    D.preuves.push(newPreuve);
...
function renderControles() {
    const today = new Date();
    let rows: { type: string; id: string; label: string; recurrence?: string; dernier?: string; prochain?: Date | null; enRetard?: boolean; expiration?: string; expired?: boolean; soonDays?: number }[] = [];
    ...
    if (rows.length === 0) {
        ...
        if (retards > 0) h += `<div class="ct-mb-3 ct-p-2 ct-r-sm" style="border:1px solid var(--ct-critical);background:var(--ct-critical-tint)"><p class="ct-text-critical ct-strong ct-m-0">${t("comp.ctrl.alertes", {count: retards})}</p></div>`;
...
async function renderHistory() {
    await _renderSnapshotsPanel({
        target: "history-content",
        orgField: "societe",
        keys: {
            create: "comp.hist.btn_creer",
            encrypt: "comp.hist.btn_chiffrer",
            decrypt: "comp.hist.btn_dechiffrer",
            none: "comp.hist.aucun",
            col_name: "comp.hist.col_nom",
            col_date: "comp.hist.col_date",
            col_org: "comp.hist.col_org",
            col_actions: "comp.hist.col_actions",
            restore: "comp.hist.btn_restaurer",
            export: "comp.hist.btn_exporter",
            hint: "comp.hist.note"
        }
    });
}
...
function _doImportEbiosRM(event: Event) {
    const file = (event.target as HTMLInputElement).files![0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = function() {
        try {
            const ebios = JSON.parse(new TextDecoder().decode(new Uint8Array(reader.result as ArrayBuffer)));
            ...
            const mesureIdMap: Record<string, string> = {};  // old EBIOS ID → new compliance ID
            if (Array.isArray(ebios.measures)) {
                ebios.measures.forEach((em: any) => {
            ...
            function _cleanDesc(text: string) {
                return text.replace(/^MES-\d+\s*[-–—]\s*/, "").trim();
            }

            // Parse the mesures_prevues field (free text) to find and link the measures
            function _linkMesuresFromText(entry: ComplianceExigence, mesuresPrevuesText?: string) {
            ...
            if (Array.isArray(ebios.socle_anssi) && ebios.socle_anssi.length > 0) {
                ...
                ebios.socle_anssi.forEach((src: any, i: number) => {
            ...
            if (Array.isArray(ebios.socle_iso) && ebios.socle_iso.length > 0) {
                ...
                ebios.socle_iso.forEach((src: any, i: number) => {
            ...
            if (ebios.socle_complementaires && typeof ebios.socle_complementaires === "object") {
                for (const [fwId, fwData] of Object.entries(ebios.socle_complementaires)) {
                    ...
                    for (const [ref, entry] of Object.entries(fwData as Record<string, any>)) {
...
try {
    ...
    // requirements view with the row targeted; #nonconformities the register.
    var _reqKey = "";
    try { _reqKey = new URLSearchParams(location.search).get("req") || ""; } catch (e) { _reqKey = ""; }
    if (_reqKey) _openRequirement(_reqKey);
    else if (location.hash === "#nonconformities") selectPanel("nonconformities");
} catch(e: any) {
