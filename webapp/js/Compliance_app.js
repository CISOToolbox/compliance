...
function renderAll() {
    ...
    if (tr)
        tr.innerHTML = _getSettingsButtonHTML() + _getGithubLinkHTML("https://github.com/CISOToolbox/compliance");
...
function _ncSpec() {
    return {
        records: function () { if (!Array.isArray(D.nonconformities))
            D.nonconformities = []; return D.nonconformities; },
        derogations: function () { if (!Array.isArray(D.derogations))
            D.derogations = []; return D.derogations; },
        ...
        setMaxDays: function (days) { _saveState(); D.meta.max_derogation_days = days; },
...
function _ncOptions() {
    ...
    opts.onChange = function () { _autoSave(); if (_currentFw && _currentSubview)
        _renderFwView(_currentFw, _currentSubview); };
    return opts;
}
function renderNonconformities() {
    if (!window.ct_nonconformity)
        return;
...
function _showPreuveModal() {
    ...
    h += '<label class="fs-xs">' + esc(t("comp.prv.label_kind")) + ' <select data-change="_updatePreuveField" data-args=\'' + _da(p.id, "kind") + '\' data-pass-value>'
        ...
        + '</select></label>';
    h += '<label class="fs-xs">' + esc(t("comp.prv.label_owner")) + ' <input type="text" value="' + esc(String(p.owner || "")) + '" data-change="_updatePreuveField" data-args=\'' + _da(p.id, "owner") + '\' data-pass-value /></label>';
    h += '</div>';
    h += '<div class="ct-flex ct-gap-2 ct-row-wrap ct-mb-2">';
...
try {
    ...
    var _reqKey = "";
    try {
        _reqKey = new URLSearchParams(location.search).get("req") || "";
    }
    catch (e) {
        _reqKey = "";
    }
    if (_reqKey)
        _openRequirement(_reqKey);
    else if (location.hash === "#nonconformities")
